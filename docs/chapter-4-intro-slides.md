...
`關鍵行為統計` 是 filter count，不是完整 conversion rate。pipeline 是否健康，需要搭配 Kafka Connect 狀態與 E2E 檢查判斷。
...
```text
converter = bytes 與 ConnectRecord 之間的轉換器
```
...
# Component 9：Delivery Semantics
