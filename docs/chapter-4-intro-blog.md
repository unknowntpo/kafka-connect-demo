# 用限量折價券搶領觀測理解 Kafka Connect 資料管線設計

電商平台常用限量折價券活動刺激短時間流量。當平台宣布今晚八點推出限量折價券，並在八點開放搶購，營運與工程團隊需要近即時觀察流量變化、領券成功與失敗、售罄原因與地區分布。這類需求看起來像 dashboard 問題，實際上會牽涉事件設計、資料管線、Kafka、Kafka Connect、Elasticsearch 與 Kibana 的分工。
...
這個問題可以從業務洞見開始，逐步推導到指標、事件欄位、資料管線與 Kafka Connect 的設計問題。

## 1. 從想得到的洞見開始

假設平台今晚八點推出限量折價券，並在八點開放搶購。營運與工程團隊需要快速判斷：

- 折價券活動流量是否正在快速上升？
- 領券失敗原因是否集中在售罄或限流？
- 哪些地區的流量壓力最高？

這些判斷就是 dashboard 要提供的洞見。設計順序可以整理為：
...
第一類是正式活動結果。活動系統通常關心使用者是否符合資格、折價券是否扣除、領券紀錄是否成立。這些資料代表正式業務狀態。

第二類是活動觀測狀態。這類指標需要分析事件流，例如每分鐘有多少瀏覽、重新整理是否突然上升、領券失敗是否集中在某個時間點、售罄後使用者是否仍大量重試。

這個 demo 聚焦第二類，也就是限量折價券搶領狀態的近即時觀測。

## 3. 這個 demo 選定的觀測指標

這個 demo 關心「熱門商品帶動的限量折價券是否正在爆量，以及爆量後發生什麼事」。可觀測指標整理如下：
...
| 欄位 | 範例值 | 支援的指標 | 用途 |
| --- | --- | --- | --- |
| `event_type` | `PAGE_REFRESHED`, `COUPON_CLAIM_FAILED` | 事件類型、關鍵行為統計 | 區分瀏覽、重新整理、排隊、成功、失敗等事件 |
| `occurred_at` | `2026-05-01T20:00:15Z` | 每分鐘流量、時間序列統計 | 把事件放到時間軸上，才能做每分鐘或每段時間統計 |
| `user_id` | `user_01883` | 高頻操作線索 | 找出是否有少數使用者大量重新整理頁面或重試 |
| `coupon_id` | `coupon_may_sale` | 指定折價券觀測 | 確認觀察的是哪張折價券 |
| `remaining_coupons` | `42`, `0` | 售罄壓力、成功與失敗比例 | 判斷失敗是否和折價券數量耗盡有關 |
| `failure_reason` | `COUPON_SOLD_OUT`, `RATE_LIMITED` | 失敗原因 | 區分售罄、限流等失敗類型 |
| `metadata.region` | `TW-NORTH`, `TW-SOUTH` | 地區流量 | 比較不同地區的流量壓力 |

`product_id` 可以額外保留為活動背景，用來標記這張折價券由哪個熱門商品帶動；這條 pipeline 的主要觀測維度仍是 `coupon_id` 與 `event_type`。

Dashboard 想回答的問題，會直接決定 event model 的欄位設計。

## 5. Service 背後的工作分工

限量折價券搶領觀測看起來是一個 dashboard，背後至少包含四種工作：
...
處理領券流程時通常也會產生事件。差別在於用途：

| 輸出 | 用途 |
| --- | --- |
| 活動結果 | 保存正式業務狀態，例如領券紀錄、剩餘券數、限流結果 |
| 事件紀錄 | 送進資料管線，用於觀測、搜尋與統計 |

因此，領券流程可以同時寫入正式活動狀態並送出 event；後續架構設計要決定的是，這些 event 由誰保存、轉換、查詢與聚合。
...
正式狀態 Database 適合承擔「處理正式狀態」與「保存活動結果」。例如領券紀錄、剩餘券數、使用者資格與限流結果。這些資料需要正確性與一致性。

如果同一個正式狀態 DB 還要承擔「查詢事件」與「聚合統計」，折價券搶領爆量時會出現幾個風險：

- 大量查詢可能影響領券流程。
- event-style 查詢和正式狀態 DB 的主要設計目標不同。
- 每分鐘聚合、失敗原因統計、地區流量分析會和正式狀態 workload 混在一起。
- 長期事件明細、查詢索引與 dashboard 聚合會增加正式狀態 DB 的容量、索引與查詢負擔。
...
這不是說稽核資料不能放在正式狀態 DB。實務上，領券紀錄、扣券結果與必要審計欄位通常仍會保存在正式資料庫，因為它們是業務事實的一部分。需要分離的是「為了查詢與觀測而保留的大量事件明細、搜尋索引與時間序列聚合」。這類資料常會複製到 read-optimized store、log store、data warehouse、Elasticsearch、OpenSearch、ClickHouse 或其他觀測平台。

這個判斷會自然導向第二個問題：事件查詢與聚合統計要放在哪裡處理？它們需要的能力和正式狀態 DB 不同，更接近事件搜尋、時間序列統計與 dashboard 查詢。
...
Demo 會產生一組固定劇本的折價券搶領流量。Dashboard 可以看到：
...
`關鍵行為統計` 是依條件分組後的事件數，目前用來比較成功、失敗與 `PAGE_REFRESHED`。它不包含所有事件類型；完整領券成功率需要額外定義嘗試次數與成功次數。
...
Elasticsearch 可以儲存巢狀 JSON；這裡選擇展平，是因為 dashboard 常見的分組與聚合操作更適合直接使用一層欄位。把 `metadata.region` 轉成 `metadata_region` 後，Kibana panel 可以直接依地區統計事件量。
...
這個 demo 用熱門商品帶動限量折價券搶領的情境，把這五個問題串在一起：
