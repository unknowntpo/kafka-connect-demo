...
| 欄位 | 範例值 | 支援的指標 | 用途 |
| --- | --- | --- | --- |
| `event_type` | `PAGE_REFRESHED`, `COUPON_CLAIM_FAILED` | 事件類型、行為統計 | 區分瀏覽、重新整理、排隊、成功、失敗等事件 |
...
| 行為統計 | 看瀏覽、重新整理、排隊、成功、失敗的事件數 |
| 失敗原因 | 看售罄或限流是否集中 |
| 高頻操作線索 | 觀察短時間內反覆重新整理或多次領券失敗是否集中於少數使用者 |
| 地區流量 | 比較不同地區的壓力分布 |

`行為統計` 是依五種 `event_type` 分組後的事件數，加總應等於事件總數。它仍不是完整領券成功率；完整成功率需要額外定義嘗試次數與成功次數。
