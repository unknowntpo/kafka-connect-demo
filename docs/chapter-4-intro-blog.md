...
這個順序很重要。若一開始直接討論 Kafka、Elasticsearch 或 Kibana，聽眾會先看到工具名稱，卻不知道這些工具要解決哪個業務問題。比較穩定的做法是先說清楚想得到的判斷，再把判斷轉成可計算的指標。
...
先定義指標，後續才能決定事件需要包含哪些欄位、資料管線要保存哪些資訊、dashboard 要呈現哪些 panel，以及 demo 成功與否要如何判斷。
...
Demo 除了展示指令能否執行，也要呈現資料流動後的觀測結果。Dashboard 會呈現：
